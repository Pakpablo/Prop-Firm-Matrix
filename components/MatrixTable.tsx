              ...
              <Fragment key={firm.id}>
                ...
                >
                  ...
                  <td className="px-3 py-3">
                    ...
                    <div className="text-xs text-zinc-500">{firm.headquarters}</div>
                  </td>
                  <td className="px-3 py-3">
                  ...
                  <td className="px-3 py-3">
                    <StarRating
                      score={firm.trustpilotScore}
                      count={firm.trustpilotReviewsCount}
                    />
                  </td>
                  <td className="px-3 py-3 text-zinc-300">
                    {money(firm.pricing.avgEvaluationFee50k)}
                  </td>
                  <td className="px-3 py-3 text-zinc-300">
                    {firm.payoutTerms.profitSplit}
                  </td>
                  <td className="px-3 py-3 text-zinc-300">
                    {firm.drawdownRules.type} · {firm.drawdownRules.dailyLossLimit}
                  </td>
                  <td className="px-3 py-3 text-zinc-300">
                    {firm.payoutTerms.frequency}
                  </td>
                      ...
                      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                        <div>
                          ...
                          <dl className="space-y-1 text-sm">
                            <Row label="Type" value={firm.pricing.pricingType} />
                            <Row
                              label="Activation Fee"
                              value={money(firm.pricing.activationFee)}
                            />
                            <Row label="Reset Fee" value={money(firm.pricing.resetFee)} />
                            <Row
                              label="Bulk Discounts"
                              value={
                                firm.pricing.hasBulkDiscounts === null
                                  ? "—"
                                  : firm.pricing.hasBulkDiscounts
                                  ? "Yes"
                                  : "No"
                              }
                            />
                        ...
                        <div>
                          ...
                          <dl className="space-y-1 text-sm">
                            <Row
                              ...
                              value={firm.drawdownRules.minTradingDays?.toString() ?? "—"}
                            />
                            <Row
                              label="Consistency Rule"
                              value={firm.drawdownRules.consistencyRule}
                              wrap
                            />
                            <Row
                              label="Min Payout"
                              value={money(firm.payoutTerms.minPayoutAmount)}
                            />
                            <Row
                              label="Payout Cap"
                              value={firm.payoutTerms.payoutCap ?? "—"}
                              wrap
                            />
                          </dl>
                        </div>
                      ...
                      <div className="mt-5 grid grid-cols-1 gap-6 sm:grid-cols-2">
                        <div>
                          ...
                          <ul className="space-y-1.5 text-sm text-zinc-300">
                            {firm.uniqueSellingPoints.map((point, i) => (
